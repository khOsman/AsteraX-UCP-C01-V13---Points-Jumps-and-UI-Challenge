...
public class Asteroid : MonoBehaviour
{
    ...
    //Need ScoreController refrence & ASO refrence
    [SerializeField]
    private ScoreController _scoreController;

    private AsteroidsScriptableObject _asteroidSO;
    ...
    private void Awake()
    {
        ...
        //_asteroidSO = FindObjectOfType<AsteroidsScriptableObject>();
        //Initiating the PARENT Asteroid ScriptableObject
        _asteroidSO = FindObjectOfType<AsteraX>().asteroidsSO;
        _scoreController = FindObjectOfType<ScoreController>();

        _jumpController = FindObjectOfType<JumpController>();
    ...
    public void OnCollisionEnter(Collision coll)
    {
        ...
        if (otherGO.tag == "Bullet" || otherGO.transform.root.gameObject.tag == "Player")
        {
            ...
            {
                //Decres9ing the player ship jump
                _jumpController.Jumps--;
                //Respawning player ship randomly
                _playerShip.SpawnPlayerShip();
            }


            //Getting asteroid points based on their size
            _scoreController.Score += _asteroidSO.pointsForAsteroidSize[size];
            Debug.Log("Point : " + _asteroidSO.pointsForAsteroidSize[size]);
